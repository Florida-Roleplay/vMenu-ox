...
namespace MenuAPI
{
    public class Menu
    {
        ...
        /// </summary>
        private static bool IsSpacerItem(MenuItem it) => it != null && !it.Enabled && (it.Text ?? "").StartsWith("~h~");

        private void StepIndex(bool up)
        {
            if (up)
                CurrentIndex = CurrentIndex == 0 ? Size - 1 : CurrentIndex - 1;
            else
                CurrentIndex = (CurrentIndex > 0 && CurrentIndex >= Size - 1) ? 0 : CurrentIndex + 1;
        ...
        public void GoUp()
        {
            if (Visible && Size > 1)
            {
                var oldItem = filterActive ? FilterItems[CurrentIndex] : MenuItems[CurrentIndex];
                int guard = 0;
                do { StepIndex(true); guard++; } while (IsSpacerItem(GetCurrentMenuItem()) && guard < Size);
                var currItem = GetCurrentMenuItem();
        ...
        public void GoDown()
        {
            if (Visible && Size > 1)
            {
                var oldItem = filterActive ? FilterItems[CurrentIndex] : MenuItems[CurrentIndex];
                int guard = 0;
                do { StepIndex(false); guard++; } while (IsSpacerItem(GetCurrentMenuItem()) && guard < Size);
                var currItem = GetCurrentMenuItem();
                IndexChangeEvent(this, oldItem, currItem, oldItem.Index, CurrentIndex);
#if FIVEM
                PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", false);
